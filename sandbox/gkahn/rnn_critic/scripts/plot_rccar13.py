...
all_exps = [load_experiments(range(i, i + 2)) for i in range(481, 552, 2)]
...
for i, exps in enumerate(list(np.array_split(all_exps[:-(len(all_exps) % 25)], len(all_exps) // 25)) + [all_exps[-(len(all_exps) % 25):]]):
    ...
    f_cumreward.savefig(os.path.join(SAVE_FOLDER, 'rccar13_cumreward_{0}.png'.format(i)), bbox_inches='tight', dpi=150)
    f_distance.savefig(os.path.join(SAVE_FOLDER, 'rccar13_distance_{0}.png'.format(i)), bbox_inches='tight', dpi=150)
    f_coll.savefig(os.path.join(SAVE_FOLDER, 'rccar13_coll_{0}.png'.format(i)), bbox_inches='tight', dpi=150)
    f_value.savefig(os.path.join(SAVE_FOLDER, 'rccar13_value_{0}.png'.format(i)), bbox_inches='tight', dpi=150)
