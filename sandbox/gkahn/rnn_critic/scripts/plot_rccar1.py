...
f_distance, axes_distance = plt.subplots(2, 3, figsize=(12, 8), sharey=True, sharex=True)
...
for ax_cumreward, ax_distance, exp in zip(axes_cumreward.ravel(), axes_distance.ravel(), all_exps):
    ...
    if len(exp) > 0:
        ...
        for ax in (ax_cumreward, ax_distance):
            ax.set_title('{0}, N: {1}, H: {2}, exp: {3}'.format(
                params['policy']['class'],
                params['policy']['N'],
                params['policy']['H'],
...
f_cumreward.savefig(os.path.join(SAVE_FOLDER, 'rccar1_cumreward.png'), bbox_inches='tight', dpi=150)
f_distance.savefig(os.path.join(SAVE_FOLDER, 'rccar1_distance.png'), bbox_inches='tight', dpi=150)
