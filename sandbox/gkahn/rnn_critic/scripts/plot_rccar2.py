...
all_exps = [load_experiments(range(i, i+2)) for i in list(range(23, 30, 2))]
...
f_cumreward, axes_cumreward = plt.subplots(2, 2, figsize=(8, 8), sharey=True, sharex=True)
f_distance, axes_distance = plt.subplots(2, 2, figsize=(8, 8), sharey=True, sharex=True)
    ...
    if len(exp) > 0:
        try:
            ...
            for ax in (ax_cumreward, ax_distance):
                ax.set_title('{0}, N: {1}, H: {2}\ntrain every: {3}, target every: {4}'.format(
                    params['policy']['class'],
                    params['policy']['N'],
                    params['policy']['H'],
...
f_cumreward.savefig(os.path.join(SAVE_FOLDER, 'rccar2_cumreward.png'), bbox_inches='tight', dpi=150)
f_distance.savefig(os.path.join(SAVE_FOLDER, 'rccar2_distance.png'), bbox_inches='tight', dpi=150)
