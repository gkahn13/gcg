...
all_exps = [load_experiments(range(i, i + 3)) for i in range(128, 451, 3)]
...
for i, exps in enumerate(list(np.array_split(all_exps[:-(len(all_exps) % 25)], len(all_exps) // 25)) + [all_exps[-(len(all_exps) % 25):]]):
    ...
    f_distance, axes_distance = plt.subplots(5, 5, figsize=(15, 15), sharey=True, sharex=True)
    f_coll, axes_coll = plt.subplots(5, 5, figsize=(15, 15), sharey=True, sharex=True)
    f_value, axes_value = plt.subplots(5, 5, figsize=(15, 15), sharey=True, sharex=True)

    for ax_cumreward, ax_distance, ax_coll, ax_value, exp in zip(axes_cumreward.ravel(), axes_distance.ravel(), axes_coll.ravel(), axes_value.ravel(), exps):
        ...
        if len(exp) > 0:
            try:
                ...
                for ax in (ax_cumreward, ax_distance, ax_coll, ax_value):
                    ax.set_title('{0}, update targ: {1}, targ K: {2}\ntrain: {3}, explore: {4}, lr: {5}'.format(
                        params['exp_name'],
    ...
    f_cumreward.savefig(os.path.join(SAVE_FOLDER, 'rccar11_cumreward_{0}.png'.format(i)), bbox_inches='tight', dpi=150)
    f_distance.savefig(os.path.join(SAVE_FOLDER, 'rccar11_distance_{0}.png'.format(i)), bbox_inches='tight', dpi=150)
    f_coll.savefig(os.path.join(SAVE_FOLDER, 'rccar11_coll_{0}.png'.format(i)), bbox_inches='tight', dpi=150)
    f_value.savefig(os.path.join(SAVE_FOLDER, 'rccar11_value_{0}.png'.format(i)), bbox_inches='tight', dpi=150)
