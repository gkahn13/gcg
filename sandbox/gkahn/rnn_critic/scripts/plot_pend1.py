...
for i in [201, 730, 270, 781, 675, 435, 841, 612]:
    try:
...
Q_thresh = edict[201]
Q_N_et_thresh = edict[730]
Q_N_wb_thresh = edict[270]
# Q_rnn_et_thresh = edict[781]
# Q_rnn_wb_thresh = edict[675]
# Q_mb_thresh = edict[435]
Q_mac_et_thresh = edict[841]
Q_mac_wb_thresh = edict[612]

width = 0.4
xs = [0,
      1-1.05*width/2., 1+1.05*width/2.,
      2-1.05*width/2., 2+1.05*width/2.]
thresh_means, thresh_stds = zip(*(Q_thresh, Q_N_et_thresh, Q_N_wb_thresh, Q_mac_et_thresh, Q_mac_wb_thresh))
colors = ['k', 'g', 'g', 'b', 'b']
...
ax.legend(loc='upper right', handles=(et_patch, wb_patch), bbox_to_anchor=(1, 1))

f.savefig(os.path.join(SAVE_FOLDER, 'pend1_comparison.png'), bbox_inches='tight', dpi=200)
plt.close(f)
