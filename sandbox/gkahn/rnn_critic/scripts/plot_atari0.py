...
def load_experiments(indices):
    ...
    for i in indices:
        try:
            exps.append(AnalyzeRNNCritic(os.path.join(EXP_FOLDER, 'hopper{0:03d}'.format(i)),
                                         clear_obs=False,
...
all_exps = [load_experiments(range(i, i+3)) for i in list(range(14, 28, 3))]
...
f_cumreward.savefig(os.path.join(SAVE_FOLDER, 'hopper0_comparison.png'), bbox_inches='tight', dpi=150)
