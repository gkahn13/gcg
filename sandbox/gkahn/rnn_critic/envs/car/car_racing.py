...
#                border[i-neg] |= border[i]
        ...
        for i in range(len(track)):
            ...
            self.road_poly_vertices.append([road1_l, road1_r, road2_r, road2_l])
        ...
        self.road_poly_vertices = np.array(self.road_poly_vertices)
        self.track = track
        return True
    ...
    def _render_road(self):
        ...
        road_poly_close = np.linalg.norm(self.road_poly_vertices - self.car.hull.position, axis=2).max(axis=1) < 70 # TODO 60 hard coded
        for (poly, color), is_close in zip(self.road_poly, road_poly_close):
            if is_close:
