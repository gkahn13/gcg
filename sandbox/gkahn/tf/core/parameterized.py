...
class Parameterized(object):
    ...
    def get_param_dtypes(self, **tags):
        ...
        if tag_tuple not in self._cached_param_dtypes:
            ...
            param_values = self.get_session().run(params)
    ...
    def set_param_values(self, flattened_params, **tags):
        ...
        self.get_session().run(ops, feed_dict=feed_dict)

    def flat_to_params(self, flattened_params, **tags):
        return unflatten_tensors(flattened_params, self.get_param_shapes(**tags))

    def get_session(self):
        return self._sess if self._sess is not None else tf.get_default_session()
    ...
    def __setstate__(self, d):
        ...
        global load_params
        if load_params:
            self.get_session().run(tf.initialize_variables(self.get_params()))
